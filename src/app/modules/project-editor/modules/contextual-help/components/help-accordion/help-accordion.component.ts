...
import { Help } from '../../constants/model/help.model'
...
export class HelpAccordionComponent implements OnInit, OnDestroy {
  ...
  onContentChange(): void {
    setTimeout(() => {
      this.elementRef.nativeElement
        .querySelectorAll('.help-img-thumb')
        .forEach((thumb) => {
          this.renderer.listen(thumb, 'click', (event) => {
            this.openModalWithComponent(event)
          })
        })
      this.elementRef.nativeElement
        .querySelectorAll('.help-video-thumb')
        .forEach((thumb) => {
          this.renderer.listen(thumb, 'click', (event) => {
            this.videoModal(event)
          })
        })
      this.elementRef.nativeElement
        .querySelectorAll('.help-pdf-thumb')
        .forEach((thumb) => {
          this.renderer.listen(thumb, 'click', (event) => {
            this.openPDF(event)
          })
  ...
  adjustHeight(): void {
    this.arrayHeight =
      'calc(100vh - ' + (this.content.length * 60 + 160) + 'px)'
  }
}
