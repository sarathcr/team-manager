...
import { TeachingStrategyEntityService } from 'src/app/modules/teacher/project-editor/store/entity/teaching-strategy/teaching-strategy-entity.service'
import {
  CustomStudentGroup,
...
export class DefinitionComponent implements OnInit, OnDestroy {
  ...
  validationCheck(): void {
    if (
      ...
      !(
        this.formData.standards?.length > 0 ||
        this.formData.customStandards?.length > 0
      ) ||
  ...
  navigateToOutput(): void {
    if (!this.buttonDisabled || this.buttonSubmitted) {
