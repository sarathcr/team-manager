...
export class EditableListComponent {
  ...
  declineDeleteModal(): void {
    if (this.overrideDelete) {
      this.declineConfirmed.emit(this.modalRef)
    } else {
      this.modalRef.hide()
    }
  }
  ...
  confirmDeleteModal(): void {
    ...
    // following code is specific to Step3 Competency objctive delete
    if (this.overrideDelete) {
      this.deleteConfirmed.emit(this.modalRef)
    } else {
      this.modalRef.hide()
    }
  }
